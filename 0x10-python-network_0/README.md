...
### 
 ...
 Write a Bash script that sends a
 `DELETE` 
...
### 
 ...
 Write a Bash script that takes in a URL as an argument, sends a
 `GET` 
 request to the URL, and displays the body of the response
 
* A header variable
 `X-School-User-Id` 
 must be sent with the value
 `98`
* You have to use
 `curl`
...
* `6-peak.txt` 
 must contain the complexity of your algorithm:
 `O(log(n)) 
 ,
 `O(n) 
 ,
...
* Your script must send a
 `POST` 
 request with the contents of a file, passed with the filename as the second argument of the script, in the body of the request
* You have to use
 `curl`
